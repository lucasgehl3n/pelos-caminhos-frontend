    ...
    <div class="py-8 px-4 mx-auto max-w-6xl lg:py-16">
      ...
      <perfect-scrollbar class="scrollable w-full px-2">
        <BadgeFilter v-if="form.idSituation" :title="situations.find(x => x.id == form.idSituation)?.name" :clicked="true"
          :class="situations.find(x => x.id == form.idSituation)?.class" :showChecked="false" :hideTimes="true">
        </BadgeFilter>
        ...
        <div class="grid gap-4 mb-4 sm:grid-cols-3 sm:gap-6 md:gap-12 sm:mb-5">
          <div class="sm:col-span-2">
            ...
            <div class="flex w-full flex-row pt-10">
              <BadgeFilter v-if="form.behavioralProfile" :title="form.behavioralProfile.name" :clicked="true"
                :hideTimes="true" v-on:click="() => { }">
              </BadgeFilter>
              <BadgeFilter v-if="form.size" :title="getSize()" :clicked="true" :hideTimes="true" v-on:click="() => { }"
                :class="'bg-green-200'">
              </BadgeFilter>
              <BadgeFilter :title="getWeight" :clicked="true" :hideTimes="true" v-on:click="() => { }"
                :class="'bg-yellow-200'">
              </BadgeFilter>
              <BadgeFilter :title="getYears" :clicked="true" :hideTimes="true" v-on:click="() => { }"
                :class="'bg-purple-200'">
              </BadgeFilter>
            </div>
          ...
          <div>
            <dl>
              ...
              <dt class="mb-2 font-semibold leading-none text-gray-900 dark:text-white" v-if="form.color">
                Cor
              </dt>
              <dd class="mb-4 text-gray-500 dark:text-gray-400 sm:mb-5" v-if="form.color">
                ...
                <div class="w-10 h-10 rounded-full ring-2" :class="form.color.class">
                </div>
              </dd>

              <dt class="mb-2 font-semibold leading-none text-gray-900 dark:text-white" v-if="form.breed">
                Raça
              </dt>
